 ...
 *
 * @project sbm
 * @package DafapTcpdf/Model
 * @filesource TcpdfFonts
 * @encodage UTF-8
...
class TcpdfFonts
{
    ...
    /**
     * Renvoie un tableau des polices disponibles pour TcPdf dont les clés sont les noms internes des polices et les valeurs sont les noms visibles.
     ...
     * @throws \Exception
     * @return array 
     * <p>Chaque ligne du tableau est de la forme file => name où :</p>
     * <ul>
    ...
    public function getFonts($mono = false)
    {
        if (empty($this->fonts)) {
            ...
            foreach (glob('*.php') as $php_file) {
                ...
                if ($features === false || $features->cidfont0) continue; 
    ...
    private function getFontFeatures($php_file)
    {
        ...
        if (!isset($cw)) return false;
        
        $result = new \stdClass();
        if (isset($file)) {
            $result->font = array(basename($file, '.z') => isset($name) ? $name : basename($php_file, '.php'));
        } else {
            $result->font = array(basename($php_file, '.php') => isset($name) ? $name : basename($php_file, '.php'));
        }
