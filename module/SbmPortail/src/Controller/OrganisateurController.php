<?php
/**
 * Controlleur du portail des organisateurs
 *
 * @project sbm
...
use SbmPortail\Form\CriteresOrgForm as CriteresForm;
use SbmPortail\Model\Db\ObjectData\CriteresOrganisateur as CriteresObject;
...
class OrganisateurController extends AbstractActionController
{
    ...
    /**
     * Permet à l'organisateur d'entrer sur un autre portail en tant que partenaire
     * (commune, établissement, transporteur)
     * L'entrée sur le formulaire est en GET.
     * Le traitement du formulaire est en POST.
     */
    public function autresPortailsAction()
    {
        $prg = $this->prg();
        if ($prg instanceof Response) {
            ...
            $userFeatures = new UserFeatures($this->categorieId, $this->userId,
                $this->db_manager, $this->sansimpayes);
            // formulaires
            $formCommune = new \SbmPortail\Form\LikeCommune();
            $formCommune->setValueOptions('userId',
                $userFeatures->getQuery()
                    ->enTantQueCommune());
            $formEtablissement = new \SbmPortail\Form\LikeEtablissement();
            $formEtablissement->setValueOptions('userId',
                $userFeatures->getQuery()
                    ->enTantQueEtablissement());
            $formTransporteur = new \SbmPortail\Form\LikeTransporteur();
            $formTransporteur->setValueOptions('userId',
                $userFeatures->getQuery()
                    ->enTantQueTransporteur());
            return new ViewModel(
                [
                    'formCommune' => $formCommune->prepare(),
                    'formEtablissement' => $formEtablissement,
                    'formTransporteur' => $formTransporteur
                ]);
        } else {
            if (array_key_exists('cancel', $prg) ||
                ! StdLib::getParam('userId', $prg, false)) {
                return $this->homePage();
            }
            // mise en session et aiguillage
            switch (StdLib::getParam('nature', $prg, false)) {
                case 'commune':
                    Session::set('commune', $prg['userId'], 'enTantQue');
                    $message = 'Accès en tant que commune.';
                    break;
                case 'etablissement':
                    Session::set('etablissement', $prg['userId'], 'enTantQue');
                    $message = 'Accès en tant qu\'établissement.';
                    break;
                case 'transporteur':
                    Session::set('transporteur', $prg['userId'], 'enTantQue');
                    $message = 'Accès en tant que transporteur.';
                    break;
                default:
                    break;
            }
            $message .= ' Pour revenir à votre espace utilisez le menu Bienvenue.';
    ...
    public function serviceGroupAction()
    {
        $result = $this->prepareListeEleves('service-group',
            function () {
                return $this->redirect()
                    ->toRoute('sbmportail/organisateur',
                    [
                        'action' => 'services',
                        'page' => $this->params('id')
                    ]);
        ...
        $view->setTemplate('sbm-portail/organisateur/eleves.phtml');
        return $view;
    }
    ...
    private function prepareListeEleves(string $sessionNameSpace, callable $fncBack)
    {
        ...
        $criteres_form = new CriteresForm();
    ...
    {
        $retour = $this->url()->fromRoute('sbmportail/organisateur');
