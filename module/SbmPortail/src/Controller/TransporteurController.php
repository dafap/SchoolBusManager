...
use SbmPortail\Form\CriteresTransporteurForm as CriteresForm;
use SbmPortail\Model\Db\ObjectData\CriteresTransporteur as CriteresObject;
use SbmPortail\Model\User\Transporteur as UserFeatures;
...
class TransporteurController extends AbstractActionController
{
    ...
    public function indexAction()
    {
        ...
        return new ViewModel(
            [
                'data' => $userFeatures->getData(),
                'transporteurs' => $userFeatures->getArrayTransporteurs()
            ]);
    }
        ...
        $viewmodel = new ViewModel(
            [
                ...
                'ptEtablissements' => $this->db_manager->get(
                    'Sbm\Portail\Transporteur\Query')
                    ->setProjection($this->projection)
                    ->setTransporteurId($userFeatures->getTransporteurIds())
        ...
        $viewmodel = new ViewModel(
            [
                'scheme' => $this->getRequest()
                    ->getUri()
                    ->getScheme(),
                'ptStations' => $this->db_manager->get('Sbm\Portail\Transporteur\Query')
                    ->setProjection($this->projection)
                    ->setTransporteurId($userFeatures->getTransporteurIds())
                    ->stationsPourCarte(),
                'config' => StdLib::getParam('station', $this->config_cartes),
    ...
    public function lignesAction()
    {
        ...
        return new ViewModel(
            [
                ...
                'millesime' => $millesime,
                'as' => $as,
                'transporteur' => $userFeatures->listeDesNoms()
            ]);
    }
    ...
    public function serviceGroupAction()
    {
        $result = $this->prepareListeEleves('service-group',
            function () {
                return $this->redirect()
                    ->toRoute('sbmportail/transporteur',
                    [
                        'action' => 'services',
                        'page' => $this->params('id')
                    ]);
        ...
        $view = new ViewModel(
            [
                ...
                'page' => $this->params('page', 1),
                'transporteur' => $result['userFeatures']->listeDesNoms(),
        ...
        $view->setTemplate('sbm-portail/transporteur/eleves.phtml');
        return $view;
    }
        ...
        return new ViewModel(
            [
                ...
                'service' => $this->db_manager->get('Sbm\Db\Table\Services')->getRecord(
                    $args['post']),
                'arrayTransporteurIds' => $userFeatures->getTransporteurIds()
            ]);
    }
    ...
    public function elevesAction()
    {
        ...
        return new ViewModel(
            [
                ...
                'transporteur' => $result['userFeatures']->listeDesNoms(),
                'sansimpayes' => $this->sansimpayes,
                'nbTransporteurs' => $result['userFeatures']->getNbTransporteurs(),
                'ns' => 'eleves'
            ]);
    }
    ...
    private function prepareListeEleves(string $sessionNameSpace, callable $fncBack)
    {
        ...
        $criteres_form = new CriteresForm($userFeatures->getArrayTransporteurs());
        $criteres_form->setValueOptions('etablissementId',
            ...
            $this->db_manager->get('Sbm\Db\Select\Services')
                ->par($userFeatures->getTransporteurIds()))
            ->setValueOptions('stationId',
            $this->db_manager->get('Sbm\Db\Select\Stations')
                ->transporteur($userFeatures->getTransporteurIds()));
        ...
        if ($sbm_isPost) {
            ...
            if ($criteres_form->isValid()) {
                $criteres = $criteres_form->getData();
                $criteres_obj->exchangeArray($criteres);
                // configure les critères pour la sous-requête
                if (array_key_exists('serviceId', $criteres)) {
                    $userFeatures->getQuery()->setServiceId($criteres['serviceId']);
                }
                if (array_key_exists('stationId', $criteres)) {
                    $userFeatures->getQuery()->setStationId($criteres['stationId']);
                }
            }
        }
    ...
    {
        $retour = $this->url()->fromRoute('sbmportail/transporteur');
