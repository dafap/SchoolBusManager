...
class Effectif extends AbstractQuery implements FactoryInterface
{
    ...
    {
        // SELECT circuitId, count(*) FROM `sbm_t_circuits` c JOIN `sbm_t_affectations` a
        // ON c.serviceId=a.service1Id AND c.stationId=a.station1Id
        // WHERE millesime=2014 GROUP BY circuitId
        $result = [];
        if ($sanspreinscrits) {
            $filtre = [
                'inscrit' => 1,
        ...
        $rowset = $this->requeteCir(2, 'circuitId', [], 'circuitId');
        foreach ($rowset as $row) {
     ...
     * [
     * 'demandes' => ['r1' => effectif1, 'r2' => effectif2, 'ele' => effectif3],
     * 'transportes' => ['r1' => effectif4, 'r2' => effectif5, 'ele' => effectif6],
     * 'total' => ['demandes' => effectif_des_demandes, 'transportes' => effectif_des_transportes)
    ...
    {
        // SELECT organismeId, count(*) FROM `sbm_t_scolarites` s WHERE millesime=2014 GROUP BY
        // organismeId
        $result = [];
        $group = [
            'organismeId'
    ...
    {
        // SELECT service1Id serviceId, count(*) FROM `sbm_t_affectations` WHERE millesime=2014
        // GROUP BY service1Id
        $result = [];
        $rowset = $this->requeteSrv('service1Id', [
            'inscrit' => 1
        ], 'service1Id');
        foreach ($rowset as $row) {
            $result[$row['column']]['r1'] = $row['effectif'];
        }
        $rowset = $this->requeteSrv2('service', [
            'inscrit' => 1
        ], 'service2Id');
        foreach ($rowset as $row) {
            $result[$row['column']]['r2'] = $row['effectif'];
    ...
    {
        // SELECT station1Id stationId, count(*) FROM `sbm_t_affectations` WHERE millesime=2014
        // GROUP BY station1Id
        $result = [];
        $rowset = $this->requeteSrv('station1Id', [
            'inscrit' => 1
        ], 'station1Id');
        foreach ($rowset as $row) {
            $result[$row['column']]['r1'] = $row['effectif'];
        }
        $rowset = $this->requeteSrv2('station',
            $this->arrayToWhere(null,
                [
                    'inscrit' => 1,
                    'isNotNull' => [
                        'a.service2Id'
    ...
    {
        // SELECT service1Id serviceId, count(*) FROM `sbm_t_affectations` a JOIN `sbm_t_services`
        // s ON a.service1Id=s.serviceId WHERE millesime=2014 AND transporteurId=1 GROUP BY
        // service1Id
        $result = [];
        if ($sanspreinscrits) {
            $filtre = [
    ...
    {
        // SELECT transporteurId, count(*) FROM `sbm_t_affectations` a JOIN sbm_t_services s on
        // a.service1Id=s.serviceId WHERE millesime=2014 GROUP BY transporteurId
        $result = [];
        if ($sanspreinscrits) {
            $filtre = [
                's.inscrit' => 1,
    ...
    {
        $select1 = new Select();
        $select1->from($this->tableName['affectations'])->where(
            [
                'millesime' => $this->millesime,
                'correspondance' => 2
            ]);
        ...
        if (is_array($conditions)) {
            ...
            $where = $conditions->equalTo('a.millesime', $this->millesime)->isNull(
                'correspondances.millesime');
        }
        $select = $this->sql->select();
        $select->from([
    ...
    private function requeteTrSrv($rang, $column, $where, $group)
    {
        ...
        ], sprintf('a.service%sId=s.serviceId', $rang),
            [
                'serviceId' => $column,
                'effectif' => new Expression('count(*)')
            ])
