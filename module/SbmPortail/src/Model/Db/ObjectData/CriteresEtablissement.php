...
class CriteresEtablissement extends SbmCommunCriteres
{
    ...
    public function getWhere($strict = [], $alias = [])
    {
        ...
        if (! empty($this->data['serviceId'])) {
            list ($moment, $ligneId) = explode('|', $this->data['serviceId']);
            $where->equalTo('aff.moment', $moment)->equalTo('aff.ligne1Id', $ligneId);
        }
        if (! empty($this->data['stationId'])) {
            $where->nest()
                ->nest()
                ->literal('aff.moment = 1')
                ->equalTo('aff.station1Id', $this->data['stationId'])
                ->unnest()->or->nest()
                ->literal('aff.moment > 1')
                ->equalTo('aff.station2Id', $this->data['stationId'])
                ->unnest()
                ->unnest();
        }
        return $where;
