...
class Liste extends AbstractQuery implements FactoryInterface
{
    ...
    private function select($columns = [], $order = null, $distinct = true)
    {
        ...
        ], 'ele.eleveId=sco.eleveId', empty($columns['sco']) ? [
            'inscrit',
            'paiement',
            'fa',
            'gratuit'
        ...
        ], 'sco.etablissementId = eta.etablissementId', empty($columns['eta']) ? [
            'etablissement' => 'nom'
        ] : $columns['eta'])
            ->join([
            'cla' => $this->db_manager->getCanonicName('classes', 'table')
        ], 'sco.classeId = cla.classeId', empty($columns['cla']) ? [
            'classe' => 'nom'
        ] : $columns['cla'])
            ->join([
            'aff' => $this->db_manager->getCanonicName('affectations', 'table')
        ], 'aff.millesime=sco.millesime And sco.eleveId=aff.eleveId', empty($columns['aff']) ? ['service1Id', 'service2Id'] : $columns['aff'])
            ->join([
            'res' => $this->db_manager->getCanonicName('responsables', 'table')
        ], 'res.responsableId=aff.responsableId', empty($columns['res']) ? [] : $columns['res'])
            ->join([
            'comres' => $this->db_manager->getCanonicName('communes', 'table')
        ], 'res.communeId=comres.communeId', empty($columns['comres']) ? [] : $columns['comres'])
            ->join([
            'comsco' => $this->db_manager->getCanonicName('communes', 'table')
        ], 'comsco.communeId=sco.communeId', empty($columns['comsco']) ? [] : $columns['comsco'], Select::JOIN_LEFT)
            ->join(['sta1' => $this->db_manager->getCanonicName('stations', 'table')], 'sta1.stationId=aff.station1Id', empty($columns['sta1']) ? [] : $columns['sta1'], Select::JOIN_LEFT)
            ->join(['sta2' => $this->db_manager->getCanonicName('stations', 'table')], 'sta2.stationId=aff.station2Id', empty($columns['sta2']) ? [] : $columns['sta2'], Select::JOIN_LEFT);
     ...
     * @return \Zend\Db\Adapter\Driver\ResultInterface
     */
    public function byEtablissementService($millesime, $etablissementId, $serviceId, $order = ['nom', 'prenom'])
    {
    ...
    public function paginatorByEtablissementService($millesime, $etablissementId, $serviceId, $order = ['nom', 'prenom'])
    {
        $select = $this->selectByEtablissementService($millesime, $etablissementId, $serviceId, $order);
        return new Paginator(new DbSelect($select, $this->db_manager->getDbAdapter()));
    }
     ...
     * @return \Zend\Db\Adapter\Driver\ResultInterface
     */
    public function byTransporteur($millesime, $filtre, $order = ['commune', 'nom', 'prenom'])
    {
    ...
    public function paginatorByTransporteur($millesime, $filtre, $order = ['commune', 'nom', 'prenom'])
    {
        $select = $this->selectByTransporteur($millesime, $filtre, $order);
        return new Paginator(new DbSelect($select, $this->db_manager->getDbAdapter()));
    }
