...
class EnvoiMailTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testConfigMailStructure()
    {
        ...
        if (array_key_exists('transport', $this->config_mail)) {
            ...
            if (array_key_exists('mode', $this->config_mail['transport'])) {
                $this->assertContains($this->config_mail['transport']['mode'], ['smtp', 'sendmail'], 'Le mode de transport est incorrect.');
                if ($this->config_mail['transport']['mode'] == 'smtp') {
                    $this->assertArrayHasKey('smtpOptions', $this->config_mail['transport'], 'smtpOptions n\'est pas definie.');
                }
            }
        ...
        if (array_key_exists('message', $this->config_mail)) {
            ...
            $this->assertArrayHasKey('subject', $this->config_mail['message'], 'Manque le subject dans message (string)');
            $this->assertArrayHasKey('body', $this->config_mail['message'], 'Manque le paramétrage de body dans message (text, html)');
            $this->assertArrayHasKey('type', $this->config_mail['message'], 'Manque le type dans message (text/html)');
    ...
    /*
     * Ne fonctionne pas : 
     * - getListeners est deprecated, 
     * - assertListenerAtPriority prend un EventManager comme 4e paramètre et pas un SharedEventManager
     * - ça bloque si je fais un die(var_dump($sharedEvents));
     * /
    public function testRegistersOnSendMailListenerOnExpectedPriority()
    {
        ...
        $this->assertListenerAtPriority([$envoiMail, 'onSendMail'], 1, 'sendMail', $sharedEvents);
    }// */
}