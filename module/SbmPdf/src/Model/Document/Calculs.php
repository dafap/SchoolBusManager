 ...
 * %somme(num_colonne) où num_colonne est le numéro de colonne en partant de 1 pour la
 * première
 * %moyenne(num_colonne)%
 * %max(num_colonne)%
 * %min(num_colonne)%
 *
 * Pour rajouter une fonction de calcul, il suffit de construire une méthode qui porte son
 * nom (minuscules sans les % délimitant son nom dans la chaine à évaluer)
 *
 ...
 * [17, 16, 18]
 * ]);
 * $f1 = 'Nombre: %compte% lignes - Total col1: %moyenne(1)% col2: %moyenne(2)% col3:
 * %moyenne(3)%';
 * $oCalculs = new Calculs($data);
 * $oCalculs->range(0, 2);
 * echo $oCalculs->getResultat($f1) . "\n";
 * echo $oCalculs->getResultat($f1) . "\n"; // pas de ré-évaluation des calculs qui sont
 * bufferisés dans la propriété resultats
 * $oCalculs->range(3, 4);
 * echo $oCalculs->getResultat($f1) . "\n"; // ré-évaluation puisque range() a changé les
 * propriétés debut et fin
 * $oCalculs->range(0, 4);
 * echo $oCalculs->getResultat($f1) . "\n"; // ré-évaluation puisque range() a changé les
 * propriétés debut et fin
 * echo $oCalculs->getResultat($f1) . "\n"; // pas de ré-évaluation des calculs qui sont
...
class Calculs
{
     ...
     *            dans la chaine
     */
    public function __construct(ArrayObject $data, $arg_default = 1)
    {
        $this->data = $data;
     ...
     * data pour les calculs et vide le buffer des résultats.
     * Sans paramètre, tout le
     ...
     * @param int $column
     */
    public function range($debut = 0, $fin = null, $column = null)
    {
        if ($column) {
            $this->arg_default = $column;
        }
        ...
        if (! ($this->debut == $debut && $this->fin == $fin)) {
            $this->debut = $debut;
            if (is_null($fin) || $fin > $this->data->count()) {
