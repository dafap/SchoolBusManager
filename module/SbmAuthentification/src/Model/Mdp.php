...
class Mdp
{
     ...
     * Par défaut, le i, le l et le o sont interdits pour éviter les confusions entre majuscules,
    ...
    /**
     * Génère un mot de passe et le renvoie sous la forme d'une chaîne de caractères.
     * Par défaut, le mot de passe est
     * constitué de 8 lettres minuscules.
     *
     * @param number $l
     *            La longueur du mot de passe généré. Elle doit être supérieure ou égale à la somme
     ...
     *            
     * @throws Exception
     * @return string Le mot de passe en clair dans une chaîne
     */
    public function genereMdp($l = 8, $c = 0, $n = 0, $s = 0)
    {
        ...
        if (! is_int($l) || ! is_int($c) || ! is_int($n) || ! is_int($s)) {
            ...
            throw new Exception(
                __METHOD__ . ' : Les paramètres sont en dehors de la plage acceptée.');
        } else if ($c > $l) {
            throw new Exception(
                __METHOD__ .
                ' : Le nombre de lettres majuscules ne doit pas excéder la longueur du mot de passe.');
        } else if ($n > $l) {
            throw new Exception(
                __METHOD__ .
                ' : Le nombre de chiffres ne doit pas excéder la longueur du mot de passe.');
        } else if ($s > $l) {
            throw new Exception(
                __METHOD__ .
                ' : Le nombre de caractères spéciaux ne doit pas excéder la longueur du mot de passe.');
        } else if ($count > $l) {
            throw new Exception(
                __METHOD__ .
                ' : La longueur demandée ne permet pas de remplir les contraintes.');
        }
        ...
        if ($count) {
            ...
            for ($i = 0; $i < $c; $i ++) {
                array_push($tmp2,
                    substr($this->caps, mt_rand(0, strlen($this->caps) - 1), 1));
            }

            for ($i = 0; $i < $n; $i ++) {
                array_push($tmp2,
                    substr($this->nums, mt_rand(0, strlen($this->nums) - 1), 1));
            }

            for ($i = 0; $i < $s; $i ++) {
                array_push($tmp2,
                    substr($this->syms, mt_rand(0, strlen($this->syms) - 1), 1));
            }
     ...
     *            
     * @return string chaîne de longueur 60 caractères
     */
