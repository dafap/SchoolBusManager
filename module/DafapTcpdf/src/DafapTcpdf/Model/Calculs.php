...
class Calculs
{
     ...
     *      Colonne sur laquelle porteront les calculs si elle n'est pas précisée dans la chaine
     */
    ...
    /**
     * Initialise la première et la dernière ligne à prendre en compte dans le tableau des data pour les calculs et vide le buffer des résultats.
     * Sans paramètre, tout le tableau des data sera pris en compte de la première à la dernière ligne.
     * Si le second paramètre est null ou s'il est supérieur à la taille du tableau des data il sera initialisé à la dernière ligne du tableau des data
     * 
    ...
    {
        if (empty($column)) $column = $this->arg_default;
        $colonne = $column - 1;
        $somme = 0;
        for ($ligne = $this->debut; $ligne <= $this->fin; $ligne ++) {
            if (!isset($this->data[$ligne][$colonne])) {
    ...
    /**
     * Renvoie la plus grande valeur de la colonne précisée dans l'intervalle fixé par la méthode range.
     *
     * @param int $column
     *      Numéro de la colonne en partant de 1
     *
    ...
    /**
     * Renvoie la plus petite valeur de la colonne précisée dans l'intervalle fixé par la méthode range.
     *
     * @param int $column
     *      Numéro de la colonne en partant de 1
     *
