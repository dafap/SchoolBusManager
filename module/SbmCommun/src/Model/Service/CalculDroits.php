...
class CalculDroits implements FactoryInterface
{
    ...
    public function createService(ServiceLocatorInterface $serviceLocator)
    {
        ...
        $this->oDistanceEtablissement = $serviceLocator->get(
    ...
    private function distancesDistrict($eleveId)
    {
        ...
        switch ($niveau) {
            case 8:
                // lycée
                $distances = $this->oDistanceEtablissement->plusieursOriginesUneDestination(
            ...
            case 4:
                // collège
                $result = $this->oDistanceEtablissement->domicilesCollege($domiciles,
                    $destination);
                $j = 1;
            ...
            default:
                // école
                $result = $this->oDistanceEtablissement->domicilesEcole($niveau,
                    $domiciles, $destination);
                $j = 1;
                foreach ($result['distances'] as $distance) {
                    $this->distance['R' . $j ++] = round($distance / 1000, 1);
    ...
    public function estEnAttente($row)
    {
        ...
        return $maxDistance < 1 || ($row['district'] == 0 && $row['derogation'] == 0);
    }
}