...
class DistanceEtablissements
{
    ...
    /**
     * Cette méthode construit une structure donnant d'une part les coordonnées LatLng des établissements et
     * d'autre part l'identité des établissements (etablissementId, communeId, statut) rangés dans le même
    ...
    /**
     * Renvoie un tableau simple des etablissementId pour lesquels il y a un droit au transport scolaire
     *
     * @param Point $domicile
     *            coordonnées du domicile dans le système utilisé ou en degré, grade ou radian (il y aura conversion)
     *            
     * @throws Exception
     * @return array tableau simple de tableaux associatifs array('etablissementId' => ..., 'distance' => ...)
     */
    ...
    public function collegesPrisEnCompte(Point $domicile)
    {
        ...
        foreach ($rowset as $clg) {
            if (in_array($clg->etablissementId, $listePublic) || ! $clg->statut) {
                ...
                $structure['tEtablissements'][] = array(
     ...
     * @param Point $college            
     *
     * @return array tableau associatif de la forme array('droit' => boolean, 'distances' => array())
     */
    ...
    public function domicilesEcole($niveau, $domiciles, $ecole)
    {
        $droit = false;
        $distances = array();
        // l'école est-elle de la commune d'un domicile. En même temps, construction du tableau $origines contenant les LatLng des domiciles.
        $estDeLaCommune = false;
        $origines = array();
        $j = 0;
        foreach ($domiciles as $pt) {
            if ($pt->getAttribute('communeId') == $ecole->getAttribute('communeId')) {
                $estDeLaCommune = true;
                $domicile = $j; // le domicile à prendre en compte pour le calcul de la plus courte distance
            }
            $origines[$j ++] = $this->getLatLngString($pt);
        }
