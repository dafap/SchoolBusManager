...
class ListeZoneActions extends AbstractHelper
{
     ...
     *            accepte.
     *            Cette liste est délimitée par des espaces. Le navigateur les utilise dans l'ordre
     *            dans lequel ils ont été définis.
     *            La valeur par défaut est la chaîne de caractères réservée "UNKNOWN" ; dans ce
     *            cas, l'ensemble de caractères utilisé
     *            correspond à celui du document contenant l'élément &lt;form&gt;.</li>
     *            <li><b>action</b> : L'URI du programme qui traitera les informations soumises par
     *            le formulaire.
     *            Cette valeur peut être surchargée par un attribut <i>formaction</i> sur un
     *            élément &lt;button&gt; ou &lt;input&gt;.</li>
     *            <li><b>autocomplete</b> : Cet attribut énuméré peut prendre deux valeurs
     *            <i>on</i> ou <i>off</i>. La valeur par défaut est <i>on</i>.<ul>
     *            <li><i>on</i> : Le navigateur peut remplir automatiquement les valeurs d'après
     *            les précédentes informations qu'a entrées
     *            l'utilisateur lors d'usages antérieurs du formulaire.</li>
     *            <li><i>off</i> : L'utilisateur doit remplir lui-même la valeur de chaque champ, à
     *            chaque utilisation du formulaire, ou
     *            le formulaire utilise son propre système d'auto-complétion ; le navigateur ne
     *            doit pas remplir automatiquement les valeurs.</li></ul>
     *            Les éléments du formulaire peuvent bien sûr outrepasser cette valeur via leur
     *            propre attribut <i>autocomplete</i>.</li>
     *            <li><b>class</b> : Cet attribut permet d'affecter des classes css à la balise qui
     *            se comportera alors comme un conteneur.</li>
     *            <li><b>enctype</b> : Cet attribut définit le type MIME qui sera utilisé pour
     *            encoder les données envoyées au serveur.
     ...
     *            Pour l'envoi de fichier, il vous faut utiliser la valeur
     *            <i>multipart/form-data</i>.
     *            Cette valeur peut être surchargée par l'attribut <i>formenctype</i> des éléments
     *            &lt;button&gt; ou &lt;button&gt;.</li>
     *            <li><b>method</b> : Cet attribut définit la méthode HTTP qui sera utilisée pour
     ...
     *            Cette valeur peut-être surchargée par l'attribut <i>formmethod</i> des éléments
     *            &lt;button&gt; ou &lt;input&gt;.
     *            <li><b>name</b> : Le nom du formulaire. Il doit être unique parmi tous les
     *            formulaires d'un document, et ne doit pas être
     *            une chaîne de caractères vide.</li>
     *            <li><b>id</b> : <i>pour la compatibilité avec HTML4<i>, où l'attribut <i>name</i>
     *            est déprécié et remplacé par celui-ci.</li>
     *            <li><b>novalidate</b> : Cet attribut booléen indique si le formulaire doit être
     *            validé au moment de sa soumission.
     *            Par défaut, le formulaire sera validé lors de sa soumission.
     *            Il peut être surchargé par l'attribut <i>formnovalidate</i> des éléments
     *            &lt;button&gt; ou &lt;input&gt; appartenant au formulaire.</li>
     *            <li><b>target</b> : Un nom ou un mot-clé indiquant où afficher la réponse après
     *            avoir envoyé le formulaire.
     *            Les mots-clés suivants ont un sens particulier :<ul>
     *            <li><i>_self</i> : charge la réponse dans le même contexte de navigation. Valeur
     *            par défaut.</li>
     *            <li><i>_parent</i> : charge la réponse dans le contexte de navigation parent de
     *            l'actuel.</li>
     *            <li><i>_top</i> : charge la réponse dans le contexte de navigation le plus haut
     *            (cad celui qui n'a aucun parent).
     *            S'il n'y a pas de parent, se comporte comme <i>_self</i>.</li>
     *            <li><i>_blank</i> : charge la réponse dans un nouveau contexte de
     *            navigation.</li>
     *            Cette valeur peut être surchargée par l'attribut <i>formtarget</i> des éléments
     *            &lt;button&gt; ou &lt;input&gt;.</li>
     */
     ...
     *            <li>menu : un tableau décrivant le sous-menu associé à cet onglet (voir
     *            getNavigation pour connaitre sa structure)</li></ul>
     *            
     * @return string
     */
    ...
    {
        // pour la compatibilité aves HTML 4, un attribut 'id' est toujours placé en même temps que
